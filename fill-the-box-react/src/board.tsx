...
export function Board(props: BoardProps) {
  ...
  const getHighScores = () => {
    const lastHighScores: Array<HighScore> = JSON.parse(
      localStorage.getItem(HIGH_SCORES_STORAGE_ITEM) ?? '[]',
    );
  ...
  useEffect(() => {
    if (finished) {
      ...
      if (started !== null) {
        ...
        setIsHighScore(
          currentHighScores.indexOf(newHighScore) < NUMBER_HIGH_SCORES,
        );
      }
    } else {
  ...
  useEffect(() => {
    if (sequence.length === 0) {
      ...
      calculateEnabledSquares(
        lineColumnLastSquare.line,
        lineColumnLastSquare.column,
      );
    }
  }, [sequence]);
  ...
  const addEnabledSquare = (
    line: number,
    column: number,
    newEnabledSquares: number[],
  ) => {
  ...
  const onSquareClick = (square: number) => {
    if (isEnabled(square)) {
      const newSequence = [...sequence, square];
  ...
  const lastSquare =
    sequence.length === 0 ? null : sequence[sequence.length - 1];
  const gameControlSxProps = {
    width: 'min(10vh, 10vw)',
      ...
      <div className={styles.infoPanel}>
        ...
        <div className={styles.gameControls}>
          ...
          <div className={styles.gameControl}>
            <IconButton
              aria-label="redo"
              disabled={undoSequence.length === 0}
              onClick={() => redo()}
              size={'large'}
              sx={gameControlSxProps}
            >
        ...
        <p className={styles.elapsedTime}>
          {elapsed ? formatElapsed(elapsed, finished) : ''}
        </p>
        {highScores.length > 0 && (
          <div className={styles.highScores}>
            <div className={styles.highScoresTitle}>Hall of Fame</div>
            {highScores.map((x, i) => (
              <React.Fragment key={`hof-${i}`}>
                <div className={styles.highScoreName}>{x.name}</div>
                <div className={styles.highScorePoints}>{x.score}</div>
                <div className={styles.highScoreElapsed}>
                  {formatElapsed(x.elapsed, true)}
                </div>
              </React.Fragment>
            ))}
          </div>
          ...
          <React.Fragment key={`line-${i}`}>
            {[...Array(props.columnsNumber)].map((y, j) => {
              ...
              const classNames = classnames(styles.square, {
                [styles.enabledSquare]: isEnabled(square),
                [styles.lastSquare]: square === lastSquare,
              });
              return (
                <div
                  key={`cell-${i}-${j}`}
        ...
        {isHighScore && (
          <>
            ...
            <div className={styles.boardOverlayMessage}>
              <form
                className={styles.boardOverlayForm}
                method="post"
                onSubmit={storeHighScore}
              >
                <center>
                  <p className={styles.boardOverlayTitle}>Enter your name:</p>
                </center>
                <center>
                  <input
                    className={styles.boardOverlayInput}
                    type="text"
                    name="txtName"
                    maxLength={3}
                    autoFocus
